...
class gaussian_inv_gamma_1(base_prior_new):
    def __init__(self,obj,name,shape,global_scale=1,global_df=1,gibbs=False):
    ...
    def get_out(self):
        if self.gibbs:
            w_out = (-self.w_obj * self.w_obj / self.sigma2_obj).sum() * 0.5
            out = w_out
        else:
    ...
    def setup_parameter(self,obj,name, shape):
        ...
        if self.gibbs:
            self.sigma2_obj = Variable(torch.zeros(1),requires_grad=False)
        else:
