...
# one regularized horseshoe prior ard for each hidden unit (weights entering the unit )in the layer
...
class rhorseshoe_ard_2(base_prior_new):
    ...
    def get_out_alt(self):
        ...
        for i in range(self.num_units):
            ...
            global_r2_out = log_inv_gamma_density(x=global_r2, alpha=0.5,beta=0.5) + param_dict["log_global_r2_obj"].sum()
    ...
    def setup_parameter(self, obj,name, shape):
        ...
        for i in range(self.num_units):
            ...
            log_global_r1_obj = self.agg_log_global_r1_obj[i:i+1,:]
            log_global_r2_obj = self.agg_log_global_r2_obj[i:i+1,:]
