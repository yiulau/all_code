...
class V_student_lr(bayes_model_class):
    def __init__(self,input_data,precision_type):

        super(V_student_lr, self).__init__(input_data=input_data,precision_type=precision_type)
    def V_setup(self):
        self.dim = self.input_data["input"].shape[1]
        self.num_ob = self.input_data["input"].shape[0]
        self.explicit_gradient = False
        self.need_higherorderderiv = False
        hs_prior_generator_fun = prior_generator("gaussian_inv_gamma2")
