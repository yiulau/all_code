...
for i in range(chain_l):
    ...
    out = NUTS_xhmc(q,0.1,H,leapfrog_ult,10,dG_dt,0.1,debug_dict)
...
for i in range(chain_l):
    out_a = abstract_NUTS_xhmc(init_q=q_point,epsilon=0.1,Ham=Ham,xhmc_delta=0.1,max_tdepth=10,debug_dict=debug_dict)
    q_point = out_a[0]

#print("explicit out q {}".format(q.data))
#print("abstract out q {}".format(q_point.flattened_tensor))

diff_de = ((debug_dict["abstract"]-debug_dict["explicit"])*(debug_dict["abstract"]-debug_dict["explicit"]))
print(diff_de)
exit()
diff_q = (q.data - q_point.flattened_tensor).sum()
#print("diff out q {}".format(diff_q))