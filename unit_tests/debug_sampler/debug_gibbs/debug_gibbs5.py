...
# for logistic regresison model cp is actually more efficient > gives higher ess for hyperparameter
...
mcmc_meta = mcmc_sampler_settings_dict(mcmc_id=0,samples_per_chain=10000,num_chains=4,num_cpu=4,thin=1,tune_l_per_chain=1000,
...
input_dict = {"v_fun":[v_generator],"epsilon":["dual"],"second_order":[False],"cov":["adapt"],"max_tree_depth":[10],
...
print(max(mcmc_sd_vec)/min(mcmc_sd_vec)) # val = 4.7