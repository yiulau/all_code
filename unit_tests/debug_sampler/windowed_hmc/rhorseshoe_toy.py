...
mcmc_meta = mcmc_sampler_settings_dict(mcmc_id=0,samples_per_chain=2000,num_chains=4,num_cpu=4,thin=1,tune_l_per_chain=1000,
                                   warmup_per_chain=1100,is_float=False,isstore_to_disk=False,allow_restart=False,seed=12)
...
input_dict = {"v_fun":[v_generator],"epsilon":["dual"],"second_order":[False],"cov":["adapt"],"max_L":[150],
               "evolve_t":[1.5],"metric_name":["diag_e"],"dynamic":[False],"windowed":[True],"criterion":[None]}
...
store_name = 'rhs_toy_windowed_sampler.pkl'
sampled = False
if sampled:
...
c_indices = mcmc_samples_beta["indices_dict"]["c"]
print(samples.shape)
...
posterior_mean_c = numpy.mean(samples[:,:,c_indices].reshape(-1,len(c_indices)),axis=0)

print("c diagnostics")
print(diagnostics_stan(samples[:,:,c_indices]))
print("tau diagnostics")
