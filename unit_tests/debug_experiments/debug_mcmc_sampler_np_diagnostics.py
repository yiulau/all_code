...
input_dict = {"v_fun":[V_2dnormal],"epsilon":["dual"],"second_order":[False],"cov":["adapt"],"max_tree_depth":[8],
...
#
adapt_cov_arguments = [adapt_cov_default_arguments(par_type="slow",dim=V_2dnormal(precision_type="torch.DoubleTensor").get_model_dim())]
...
out,feature_names = sampler1.np_diagnostics()
...
mcmc_cov = numpy.cov(mcmc_samples_mixed,rowvar=False)
mcmc_sd_vec = numpy.sqrt(numpy.diagonal(mcmc_cov))

print("mcmc problem difficulty")


print(max(mcmc_sd_vec)/min(mcmc_sd_vec)) # val = 2.47


out_np,feature_names = sampler1.np_diagnostics()
