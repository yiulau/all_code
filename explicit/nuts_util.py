...
def GNUTS_tensor(q_init,epsilon,H_fun,leapfrog,max_tdepth,p_sharp_fun):
    # sum_p should be a tensor instead of variable
    p = torch.randn(len(q_init))
    q_left = q_init.clone()
    q_right = q_init.clone()
    p_left = p.clone()
    p_right = p.clone()
    p_sleft = p_sharp_fun(q_init, p).clone()
    p_sright = p_sharp_fun(q_init, p).clone()
    j = 0
    q_prop = q_init.clone()
    log_w = -H_fun(q_init,p)
    sum_p = p.clone()
...
def BuildTree_nuts_xhmc_tensor(q,p,v,j,epsilon,leapfrog,H_fun,dG_dt,xhmc_delta):
    if j ==0:
        q_prime,p_prime = leapfrog(q,p,v*epsilon,H_fun)
        log_w_prime = -H_fun(q_prime, p_prime)
        ave = dG_dt(q, p)
...
def NUTS_criterion_tensor(q_left,q_right,p_left,p_right):
