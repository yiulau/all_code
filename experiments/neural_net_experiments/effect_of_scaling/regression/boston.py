from experiments.neural_net_experiments.effect_of_scaling.scaled_model_regression import V_fc_scaled_model_regression
...
v_generator =wrap_V_class_with_input_data(class_constructor=V_fc_scaled_model_regression,input_data=train_set,model_dict=model_dict)
...
# input_dict = {"v_fun":[v_generator],"epsilon":[0.01],"second_order":[False],"max_tree_depth":[6],
#                "metric_name":["unit_e"],"dynamic":[True],"windowed":[False],"criterion":["gnuts"]}
...
store_name = 'normal_regression_fc1_sampler.pkl'
sampled = False
if sampled:
...
#target_dataset = get_data_dict("8x8mnist",standardize_predictor=True)
#input_data = {"input":input_data["input"][:500,],"target":input_data["target"][:500]}

v_generator = wrap_V_class_with_input_data(class_constructor=V_fc_scaled_model_regression,input_data=train_set,model_dict=model_dict)
precision_type = "torch.DoubleTensor"

# test_mcmc_samples = numpy.zeros((1,mcmc_samples_mixed.shape[1]))
# test_mcmc_samples[0,:] = mcmc_samples_mixed[195,:]
#
#
# print("debug")
#
...
te2,predicted2 = test_error(test_set,v_obj=v_generator(precision_type=precision_type),mcmc_samples=mcmc_samples_mixed,type="regression",memory_efficient=False)
