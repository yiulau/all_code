...
store_name = 'gibbs_55_units_fc1_results.pkl'
sampled = False
if sampled:
    out = pickle.load(open(store_name, 'rb'))
else:
    mcmc_samples_weight = torch.zeros(1, num_samples, dim)
    mcmc_samples_hyper = torch.zeros(1, num_samples, 1)

    for i in range(num_samples):
        outq, out_hyperparam = update_param_and_hyperparam_one_step(init_q_point, init_hyperparam, Ham, 0.01, 50,
                                                                    log_obj)
    ...
    out = {"weight":mcmc_samples_weight,"hyper":mcmc_samples_hyper}
    with open(store_name, 'wb') as f:
        pickle.dump(out, f)


mcmc_samples_weight = out["weight"]
mcmc_samples_hyper = out["hyper"]
...
print(mcmc_samples_weight[0,5,1:10])

print((diagnostics_stan(mcmc_samples_tensor=mcmc_samples_weight)["ess"]))



test_mcmc_samples = numpy.zeros((mcmc_samples_weight.shape[1],mcmc_samples_weight.shape[2]))
